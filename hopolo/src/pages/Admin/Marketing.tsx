...
import { fetchPromoCodes, addPromoCode, deletePromoCode, PromoCode } from '../../services/promoService';
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div style={{ display: 'flex', gap: 'var(--spacing-2)' }}>
          <Button onClick={() => setIsAdding(!isAdding)}>{isAdding ? 'Cancel' : 'Add Promo Code'}</Button>
          ...
          <form className={styles.form} onSubmit={handleSave}>
            ...
            <div className={styles.row}>
              ...
              <Input 
                ...
                onChange={e => setNewPromo({ ...newPromo, minPurchase: parseFloat(e.target.value) })} 
              />
            </div>
      ...
      <Card>
        <table className={styles.table}>
          ...
          <tbody>
            {promos.map((promo) => (
              <tr key={promo.id}>
                <td className={styles.td}><strong>{promo.code}</strong></td>
                <td className={styles.td}>
                  {promo.type === 'percentage' ? `${promo.value}%` : `$${promo.value.toFixed(2)}`}
                </td>
                <td className={styles.td}>${promo.minPurchase?.toFixed(2) || '0.00'}</td>
                <td className={styles.td}>
                  <Button variant="outline" style={{ padding: '4px 8px', fontSize: '0.75rem' }} onClick={() => handleDelete(promo.id!)}>Delete</Button>
                </td>
              </tr>
            ))}
          </tbody>
