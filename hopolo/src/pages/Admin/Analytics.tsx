...
import { aggregateRevenuePerDay, calculateTopProducts, calculateCategoryShare } from '../../lib/analyticsUtils';
...
const Analytics: React.FC = () => {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter(order => {
      const date = order.createdAt?.toDate ? order.createdAt.toDate() : new Date(0);
      return now.getTime() - date.getTime() <= rangeMs;
    });
  }, [orders, range]);

  const revenueData = useMemo(() => aggregateRevenuePerDay(filteredOrders), [filteredOrders]);
  const topProductsData = useMemo(() => calculateTopProducts(filteredOrders), [filteredOrders]);
  const categoryData = useMemo(() => calculateCategoryShare(filteredOrders), [filteredOrders]);
