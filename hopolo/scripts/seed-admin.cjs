...
if (!fs.existsSync(serviceAccountPath)) {
  ...
  console.error('Please download your service account key from Firebase Console and save it as "service-account.json" in the project root, or set GOOGLE_APPLICATION_CREDENTIALS environment variable.');
  process.exit(1);
}
...
const seedUsers = async () => {
  ...
  await batch.commit();
  console.log('User profiles seeded. (Note: Auth users must be created separately)');
};
